...
public class Bishop implements PinPiece {
    ...
    public Bishop(int row, int col, boolean isWhite, String FILE_PATH) {
        ...
        if (Board.get().inStateOfCheck()) {
            ...
            if (AttackerMap.get().getSquare(destRow, destCol).containsAttacker(this)
                ...
                attacking diagonal as the piece that is pinning this bishop. */
                    if (pieceThisIsPinnedBy instanceof Queen) {
                        if (pieceThisIsPinnedBy.getRow() == row || pieceThisIsPinnedBy.getCol() == col) return false;
                        else return Helper.isValidDiagonal(pieceThisIsPinnedBy, destRow, destCol);
                    } else if (pieceThisIsPinnedBy instanceof Bishop) {
